...
export default function SidebarNavButton({ icon, label, description, to, onClick }) {
  return (
    <NavLink
      ...
      onClick={onClick}
      className={({ isActive }) =>
        `group block w-full rounded-xl border px-3 py-3 text-left transition ${isActive
    ...
    >
      {({ isActive }) => (
        <div className="flex items-center gap-3">
          <span
            className={`inline-flex h-9 w-9 items-center justify-center rounded-lg text-lg ${isActive
          ...
          <div className="min-w-0">
            ...
            <p
              className={`truncate text-xs ${isActive ? "text-blue-100" : "text-slate-400"
                }`}
            >
              {description}
            </p>
